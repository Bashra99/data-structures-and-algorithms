...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(item=>{
    let filteredArr= item.filter(item2=>{
      return (item2%5===0&&typeof item2==="number")
    })
   return filteredArr.map(item3=>{
     return Math.pow(2,item3)
   })
  })
  // Solution code here...
};
...
let findShortest = (data) => {
  // Solution code here...
  let height=96
  let x= data.filter(item=>item.height==height)
    return x[0].name
};
